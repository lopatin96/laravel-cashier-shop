...
class BasePriceCalculationStrategy implements PriceCalculationStrategy
{
    ...
    public static function getPPPForCountry(string $countryCode = 'us'): float
    {
        if (! isset(self::$countryToPPP[$countryCode])) {
            return self::getPPPForCountry();
        }

        $gdpCountry = self::$countryToPPP[$countryCode];

        $minGdp = min(array_filter(self::$countryToPPP, fn($value, $key) => $key !== 'us', ARRAY_FILTER_USE_BOTH));
        $maxGdp = max(array_filter(self::$countryToPPP, fn($value, $key) => $key !== 'us', ARRAY_FILTER_USE_BOTH));
